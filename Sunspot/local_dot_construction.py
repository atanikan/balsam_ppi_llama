...
def search_patterns_in_file(folder, proteins_to_find, output_file, write_lock):
...
def worker_main(queue, output_file, write_lock):
    while True:
...
def main():
    ...
    for _ in range(num_workers):
        p = multiprocessing.Process(target=worker_main, args=(queue, output_file, write_lock))
        p.start()
        workers.append(p)
